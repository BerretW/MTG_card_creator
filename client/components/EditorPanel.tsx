...
const EditorPanel: React.FC<EditorPanelProps> = (props) => {
    ...
    const handleCustomSymbolUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            const name = prompt("Enter a name for this set symbol:", file.name.replace(/\.[^/.]+$/, ""));
            if (name) {
    ...
    return (
        <div className="space-y-4">
            <h2 className="text-2xl font-beleren text-yellow-200 border-b-2 border-yellow-200/20 pb-2">Card Editor</h2>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Input label="Name" value={cardData.name} onChange={e => handleChange('name', e.target.value)} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Select label="Card Type" value={cardData.cardType} onChange={e => handleChange('cardType', e.target.value as CardType)}>
                 ...
                 <Input label="Subtype" value={cardData.subtype} onChange={e => handleChange('subtype', e.target.value)} placeholder="Elf Druid" />
            </div>

            <Textarea label="Rules Text" value={cardData.rulesText} onChange={e => handleChange('rulesText', e.target.value)} rows={4} />
            <Textarea label="Flavor Text" value={cardData.flavorText} onChange={e => handleChange('flavorText', e.target.value)} rows={2} />

            {cardData.cardType === CardType.Creature && (
                <div className="grid grid-cols-2 gap-4">
                    <Input label="Power" value={cardData.power} onChange={e => handleChange('power', e.target.value)} />
                    <Input label="Toughness" value={cardData.toughness} onChange={e => handleChange('toughness', e.target.value)} />
                </div>
            )}
            ...
            <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Set Symbol</label>
                <div className="flex gap-2">
                    <select value={cardData.setSymbolUrl} onChange={e => handleChange('setSymbolUrl', e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-md px-3 py-2 text-white focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 transition">
                        {allSetSymbols.map((symbol) => <option key={symbol.url} value={symbol.url}>{symbol.name}</option>)}
                    </select>
                    <input type="file" ref={customSymbolInputRef} className="hidden" accept="image/svg+xml, image/png" onChange={handleCustomSymbolUpload} />
                    <button onClick={() => customSymbolInputRef.current?.click()} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-md transition text-center text-sm flex-shrink-0">
                        Upload
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Input label="Artist" value={cardData.artist} onChange={e => handleChange('artist', e.target.value)} />
                <Input label="Collector #" value={cardData.collectorNumber} onChange={e => handleChange('collectorNumber', e.target.value)} />
            </div>

            <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Template</label>
                <div className="flex gap-2">
                    <select value={cardData.templateId} onChange={e => handleChange('templateId', e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-md px-3 py-2 text-white focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 transition">
                        {templates.map(template => <option key={template.id} value={template.id}>{template.name}</option>)}
                    </select>
                    <button onClick={onOpenTemplateEditor} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-md transition text-center">
                        Manage
                    </button>
                </div>
            </div>
