...
const TemplateEditor: React.FC<TemplateEditorProps> = ({ templates: initialTemplates, onSave, onClose }) => {
    ...
    const handleUpdateFont = (key: FontKey, newFontProps: Partial<FontProperties>) => {
        ...
        const updatedTemplate = {
            ...activeTemplate,
            fonts: { ...activeTemplate.fonts, [key]: { ...activeTemplate.fonts[key], ...newFontProps } }
        };
        handleUpdateTemplate(updatedTemplate);
    };
    ...
    const renderPropertyControls = () => {
        if (!activeTemplate || !selectedElementKey) {
            return <p className="text-gray-400">Select an element on the card to edit its properties.</p>;
        }
        ...
        const handlePropChange = (prop: keyof TemplateElement, value: string) => {
            handleUpdateElement(selectedElementKey, { ...element, [prop]: parseFloat(value) || 0 });
        };
        
        return (
            <div className="space-y-4">
                 <h4 className="text-lg font-bold capitalize text-yellow-300">{selectedElementKey} Properties</h4>
                 <div className="grid grid-cols-2 gap-2">
                     {(['x', 'y', 'width', 'height'] as const).map(prop => (
                         <div key={prop}>
                             <label className="text-xs">{prop.toUpperCase()} (%)</label>
                             <input type="number" step="0.1" value={element[prop]} onChange={e => handlePropChange(prop, e.target.value)} className="w-full bg-gray-900 border border-gray-600 rounded px-2 py-1 text-sm"/>
                         </div>
                     ))}
                 </div>
                 ...
                 {fontKey && fontProps && (
                     <div className="space-y-2 pt-2 border-t border-gray-600">
                         <h4 className="text-lg font-bold capitalize text-yellow-300">Font Properties</h4>
                         <div>
                            <label className="text-xs">Font Family</label>
                            <select value={fontProps.fontFamily} onChange={(e) => handleUpdateFont(fontKey, { fontFamily: e.target.value })} className="w-full bg-gray-900 border border-gray-600 rounded px-2 py-1 text-sm">
                         ...
                         <div className="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <input type="number" value={fontProps.fontSize} onChange={(e) => handleUpdateFont(fontKey, { fontSize: parseInt(e.target.value, 10) })} className="w-full bg-gray-900 border border-gray-600 rounded px-2 py-1 text-sm" />
                            </div>
                            <div>
                                <label className="text-xs">Color</label>
                                <input type="color" value={fontProps.color} onChange={(e) => handleUpdateFont(fontKey, { color: e.target.value })} className="w-full bg-gray-900 border-gray-600 rounded h-8" />
                            </div>
                         </div>
                         <div className="flex items-center gap-4">
                            <label className="text-xs">Bold</label>
                            <input type="checkbox" checked={fontProps.fontWeight === 'bold'} onChange={(e) => handleUpdateFont(fontKey, { fontWeight: e.target.checked ? 'bold' : 'normal' })} className="form-checkbox h-4 w-4 text-yellow-400 bg-gray-900 border-gray-600 rounded focus:ring-yellow-400" />
                         </div>
                     </div>
                 )}
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800 rounded-lg shadow-xl w-full h-full max-w-7xl max-h-[90vh] border border-gray-700 flex flex-col">
                <header className="p-4 border-b border-gray-700 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={handleSaveAndClose} className="py-2 px-6 rounded-md bg-green-600 hover:bg-green-700 transition">Save & Close</button>
                ...
                <div className="flex-grow flex overflow-hidden">
                    ...
                    <aside className="w-1/4 bg-gray-900/50 p-4 space-y-4 overflow-y-auto">
                        <div>
                            <h4 className="text-lg font-bold text-yellow-300 mb-2">Templates</h4>
                            ...
                            <button onClick={handleAddNewTemplate} className="w-full mt-2 py-2 px-4 rounded-md bg-blue-600 hover:bg-blue-700 transition text-sm">New/Duplicate</button>
                        </div>
                        <hr className="border-gray-600"/>
                        {activeTemplate && (
                           <div>
                                <h4 className="text-lg font-bold text-yellow-300 mb-2">Frame Image</h4>
                                <input type="file" accept="image/png" onChange={handleFrameUpload} className="text-sm w-full file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-gray-600 file:text-white hover:file:bg-gray-500"/>
                            </div>
                        )}
                        <hr className="border-gray-600"/>
                    ...
                    <main className="w-3/4 flex items-center justify-center p-4 bg-gray-900">
                        {activeTemplate && (
                            <div className="w-[375px] h-[525px] rounded-[18px] overflow-hidden shadow-2xl relative select-none bg-black bg-cover bg-center" style={{ backgroundImage: `url(${activeTemplate.frameImageUrl})`}}>
